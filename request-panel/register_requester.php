...
if (!empty($_POST) && $_POST['submit'] == 'submit') {
    if(isset($_POST['rid'])){
        $check = $obj->select('tbl_requester_login','*','rid',array($_POST['rid']));
        if($check){
            ...
            $obj->insert('tbl_requester_login',$_POST);
    ...
    <div class="col-md-5 m-auto">
      <div class="card text-dark border-danger mt-5" >
        ...
        <form action="" method="post" class="form-group" onsubmit="return validate();">
            ...
            <?php $donors = $obj->select('tbl_request_register','*'); ?>
           <select name="rid" id="" class="form-control">
           <?php foreach($donors as $donor){ ?>
                <option value="<?=$donor['rid'];?>"><?=$donor['name'];?></option>
            <?php } ?>
           </select>
        </div>
          <div class="form-group">
